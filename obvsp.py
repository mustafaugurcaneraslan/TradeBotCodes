...
def find_anomalies():
    """1 saatlik ve 5 dakikalık Z-skorlarını karşılaştırarak anomaliyi bulur."""
    ...
    for symbol in symbols:
        try:
            ...
            prices_5m = get_historical_klines(symbol, "15m")
            
            if len(prices_1h) < 50 or len(prices_5m) < 50:
                continue  # Yetersiz veri olanları atla
            
            # 1 saatlik Z-skoru hesapla
            avg_price_1h = np.mean(prices_1h[-20:])
            ...
            avg_price_5m = np.mean(prices_5m[-20:])
            std_dev_5m = calculate_standard_deviation(prices_5m)
            z_score_5m = calculate_z_score(prices_5m[-1], avg_price_5m, std_dev_5m)
            
            # Biri pozitif biri negatifse ekle
            if abs(z_score_5m) > 0.5 and z_score_1h*z_score_5m<0:
                anomalies.append((symbol, abs(z_score_5m), z_score_5m))
                print(f"{symbol} - 1H Z: {z_score_1h:.2f}, 5M Z: {z_score_5m:.2f}")
